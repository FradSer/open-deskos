...
This walkthrough uses sample app `application/open_deskos`, covering entry <LinkToSource relativePath="application/open_deskos/main/main.c" /> and wiring in <LinkToSource relativePath="components/common/app_claw/app_claw.c" />.

## `main.c` entry

<LinkToSource relativePath="application/open_deskos/main/main.c" /> initializes required system pieces and peripherals so the ESP-Claw Agent can run.

<Steps>
1. **Init NVS and load settings**

    On boot, `open_deskos` initializes the NVS partition (`nvs_flash_init`).
...
3. **Init Memory**
    ...
    `claw_memory_init` sets session history root (`/fatfs/sessions/`), long-term memory directory (`/fatfs/memory`), and per-message limits (`open_deskos` defaults to 4096 chars per message).
