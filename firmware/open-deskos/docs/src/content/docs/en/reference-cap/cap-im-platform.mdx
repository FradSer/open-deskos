...
Application startup prepares credentials and attachment settings per enabled platform, then registers the matching runtime groups. The `open_deskos` app binds outbound Event Router channels such as `qq`, `feishu`, `telegram`, `wechat`, and `web` to the corresponding send tools.
