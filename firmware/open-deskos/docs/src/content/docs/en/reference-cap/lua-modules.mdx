...
<Aside type="note" title="Module conditional compilation">
    In `open_deskos`, every module is controlled by Kconfig. When the firmware is not built with a module's component, Lua scripts cannot use that module.
</Aside>
...
Output roots are application-specific; in `open_deskos`, built-in Lua resources usually go to `/fatfs/scripts/builtin`.
