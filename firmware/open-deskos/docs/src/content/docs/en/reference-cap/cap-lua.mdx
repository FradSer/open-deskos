...
Writable `cap_lua` runtime scripts must live under the Script Base Dir. In `open_deskos`, the default Script Base Dir is `/fatfs/scripts/` (apps can override it via `cap_lua_set_base_dir`). Read-only scripts bundled with Skills may run from `/fatfs/skills/<skill_id>/scripts/...`.
