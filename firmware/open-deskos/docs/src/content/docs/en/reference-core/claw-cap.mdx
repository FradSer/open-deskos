...
`open_deskos` defaults to exposing `cap_files`, `cap_scheduler`, `cap_lua`, `cap_skill`, `cap_llm_inspect`, `cap_http_request`, `cap_web_search`, and `cap_router_mgr` to the LLM, and full structured-memory mode also exposes `claw_memory`;
