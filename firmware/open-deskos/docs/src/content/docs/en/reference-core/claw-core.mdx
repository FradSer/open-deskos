...
`open_deskos` registers multiple providers in fixed order inside `app_claw_start`:
...
In `open_deskos`, the `call_cap` callback targets `claw_cap_call_from_core` from `claw_cap`.
...
In `open_deskos`, `persist_context` is `claw_memory_persist_context_callback` from `claw_memory`.
