...
In `open_deskos`, `default_route_messages_to_agent` depends on whether the LLM is fully configured: if the LLM API is not set up correctly, it will be `false`.

<Aside type="caution" title="When the LLM is not configured, claw_core does not start">
  If any of the LLM API key, backend_type, or model is empty, `open_deskos` skips `claw_core_init` and `claw_core_start`, so the inference core never starts.
...
In `open_deskos`, QQ, Feishu, Telegram, WeChat, and Web (local IM) each bind their send path and `channel` name.
