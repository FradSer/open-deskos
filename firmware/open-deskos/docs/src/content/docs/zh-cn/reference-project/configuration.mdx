...
ESP-Claw `open_deskos` 使用 menuconfig (sdkconfig) 保存初始化默认配置，使用 NVS 持久化运行时修改的配置。

`open_deskos` 会将配置储存在 NVS 命名空间 `app` 里。
上电后，程序会读取 menuconfig 中的默认配置，再使用 NVS 中储存的键（如有）覆盖。
字段列表与读写逻辑位于 <LinkToSource relativePath="application/open_deskos/components/app_config/app_config.c" />。
