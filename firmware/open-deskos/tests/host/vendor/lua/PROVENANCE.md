...
| Acquired | 2026-07-11 |
| Source of these files | Copied verbatim from the fork's resolved managed component at `application/edge_agent/managed_components/georgik__lua/` (`lua/*.c`, `lua/*.h`, `include/luaconf.h`) after task 001's `idf.py build` resolved it. |
...
`5.5.0~7` (recorded in `application/edge_agent/dependencies.lock` and in
