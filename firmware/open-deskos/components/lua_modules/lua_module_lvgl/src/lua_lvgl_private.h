...
/* Dual partial buffers must fit internal DMA SRAM on P4. Empirically ~115 KiB
 * is available after one buffer; 48 lines ×2 ≈ 135 KiB total stays in SRAM. */
#define LUA_MODULE_LVGL_DEFAULT_BUFFER_LINES 48
#define LUA_MODULE_LVGL_DEFAULT_TICK_MS 2
#define LUA_MODULE_LVGL_DEFAULT_TASK_PERIOD_MS 3
#define LUA_MODULE_LVGL_TASK_STACK 16384
#define LUA_MODULE_LVGL_TASK_PRIO 5
/* Pin LVGL timer task off core 0 so DPI/DMA2D ISR + net stay responsive. */
#define LUA_MODULE_LVGL_TASK_CORE 1
...
#define LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX 8

/* Native pager lifecycle, sampled once at LV_EVENT_REFR_START so a later
 * state transition cannot misattribute a completed expensive refresh. */
typedef enum {
    LUA_LVGL_PAGER_TRACE_PHASE_IDLE = 0,
    LUA_LVGL_PAGER_TRACE_PHASE_DRAG,
    LUA_LVGL_PAGER_TRACE_PHASE_SNAP,
    LUA_LVGL_PAGER_TRACE_PHASE_RESTORE,
} lua_lvgl_pager_trace_phase_t;
...
typedef struct lua_lvgl_touch_sampler lua_lvgl_touch_sampler_t;
...
typedef struct lua_lvgl_obj_record {
    ...
    size_t data2_size;
    int value_cache;
    int32_t scroll_snapshot_page_width;
    uint16_t scroll_snap_anim_ms;
    struct lua_lvgl_obj_record *scroll_snapshot_live[LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX];
    struct lua_lvgl_obj_record *scroll_snapshot_images[LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX];
    /* Small live overlays that travel with the bitmap only while a pager is
     * moving (clock/date/timer patches). Keeping them separate avoids taking
     * a costly full-page snapshot on every timer tick. */
    struct lua_lvgl_obj_record *scroll_snapshot_overlays[LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX];
    /* Optional native pager markers. They stay in the LVGL task so a moving
     * bitmap can update its visual page affordance without queueing a Lua
     * callback for every raw scroll delta. */
    struct lua_lvgl_obj_record *scroll_snapshot_indicators[LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX];
    struct lua_lvgl_obj_record *scroll_snapshot_indicator_pager;
    uint8_t scroll_snapshot_layer_count;
    uint8_t scroll_snapshot_indicator_count;
    uint8_t scroll_snapshot_indicator_intensity[LUA_LVGL_SCROLL_SNAPSHOT_LAYER_MAX];
    int32_t scroll_snapshot_indicator_hit_width;
    int32_t scroll_snapshot_indicator_idle_width;
    int32_t scroll_snapshot_indicator_active_width;
    int32_t scroll_snapshot_indicator_height;
    lv_color_t scroll_snapshot_indicator_active_color;
    lv_color_t scroll_snapshot_indicator_idle_color;
    uint32_t generation;
    lua_lvgl_obj_type_t type;
    bool valid;
    bool scroll_snap_anim_hooked;
    bool scroll_snapshot_hooked;
    bool scroll_snapshot_active;
    bool scroll_snapshot_snap_active;
    bool scroll_snapshot_restarting;
    bool scroll_snapshot_replacement_pending;
...
/* Snapshot image widget: record->data holds one of these when data_size is
 * LUA_LVGL_SNAPSHOT_DATA_MAGIC. lua_lvgl_record_release_resources() uses
 * lv_draw_buf_destroy() (not free()) for it. */
typedef struct {
    lv_draw_buf_t *draw_buf;
    lv_image_dsc_t dsc;
} lua_lvgl_snapshot_t;

#define LUA_LVGL_SNAPSHOT_DATA_MAGIC 0x53A97001u
...
typedef struct {
    ...
    void *draw_buf;
    void *draw_buf2; /* optional second partial buffer for pipelined flush */
    size_t draw_buf_size;
    bool draw_buf_owned; /* false when buffers are DPI panel framebuffers */
    bool fb_swap_mode;   /* MIPI/RGB: FULL/DIRECT into panel FBs + swap */
    bool adapter_active; /* esp_lvgl_adapter owns display/task/tick/flush */
    bool flush_callbacks_registered;
    volatile bool flush_pending;
    /* Incremented only when a submitted DPI frame is safe to reuse. The
     * pager trace compares this sequence with native scroll timestamps to
     * measure input-to-presentation without logging in the ISR. */
    volatile uint32_t pager_trace_present_sequence;
    ...
    /* The adapter owns the production LVGL worker. This timer runs there so
     * buffered GT911 input can arm the normal indev read timer without the
     * core-0 sampler ever touching LVGL. */
    lv_timer_t *touch_bridge_timer;
    void *touch_handle;
    lua_lvgl_touch_sampler_t *touch_sampler;
} lua_lvgl_state_t;
...
int lua_lvgl_screen_load_anim(lua_State *L);
...
/* lua_lvgl_symbols.c — FontAwesome glyphs already in Montserrat. */
...
/* Mark the pointer's normal LVGL input timer ready for one queued semantic
 * sample. The caller must hold lua_lvgl_lock(); the sampler only marks pending
 * work and wakes the task, so it never calls LVGL from the I2C core. */
void lua_lvgl_indev_process_pending_locked(void);

/* Native home-pager trace hooks. They are intentionally C-only: the sampler
 * and renderer record one summary per physical scroll, without scheduling Lua
 * callbacks or printing from the 4 ms sampling path. The caller of the
 * `*_locked` functions must hold lua_lvgl_lock(). */
