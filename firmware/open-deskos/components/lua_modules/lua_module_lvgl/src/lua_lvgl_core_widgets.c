...
/* Animated screen load: screen:load_anim(type, ms, delay, auto_del). Mirrors
 * lua_lvgl_screen_load but routes through lv_screen_load_anim so the Shell can
 * use the reference App Flow instead of hard-cutting. `type` is a string name
 * (fade/over_left/move_top/...); `auto_del` defaults false so the Shell can
 * keep its resident home screen while an App screen is active. */
int lua_lvgl_screen_load_anim(lua_State *L)
{
    lua_lvgl_obj_ud_t *ud = lua_lvgl_check_ud(L, 1);
    lua_lvgl_obj_type_t type;
    const char *anim_str = luaL_optstring(L, 2, "fade");
    uint32_t ms = (uint32_t)luaL_optinteger(L, 3, 250);
    uint32_t delay = (uint32_t)luaL_optinteger(L, 4, 0);
    bool auto_del = lua_toboolean(L, 5);
    esp_err_t err = lua_lvgl_lock();
    lv_obj_t *screen;
    lv_screen_load_anim_t a = LV_SCREEN_LOAD_ANIM_FADE_ON;
    const char *obj_error = NULL;
    ...
    if (type != LUA_LVGL_OBJ_SCREEN) {
        lua_lvgl_unlock();
        return luaL_error(L, "lvgl load_anim requires a screen object");
    }
    if      (strcmp(anim_str, "none") == 0)        a = LV_SCREEN_LOAD_ANIM_NONE;
    else if (strcmp(anim_str, "over_left") == 0)   a = LV_SCREEN_LOAD_ANIM_OVER_LEFT;
    else if (strcmp(anim_str, "over_right") == 0)  a = LV_SCREEN_LOAD_ANIM_OVER_RIGHT;
    else if (strcmp(anim_str, "over_top") == 0)    a = LV_SCREEN_LOAD_ANIM_OVER_TOP;
    else if (strcmp(anim_str, "over_bottom") == 0) a = LV_SCREEN_LOAD_ANIM_OVER_BOTTOM;
    else if (strcmp(anim_str, "move_left") == 0)   a = LV_SCREEN_LOAD_ANIM_MOVE_LEFT;
    else if (strcmp(anim_str, "move_right") == 0)  a = LV_SCREEN_LOAD_ANIM_MOVE_RIGHT;
    else if (strcmp(anim_str, "move_top") == 0)    a = LV_SCREEN_LOAD_ANIM_MOVE_TOP;
    else if (strcmp(anim_str, "move_bottom") == 0) a = LV_SCREEN_LOAD_ANIM_MOVE_BOTTOM;
    else if (strcmp(anim_str, "fade_in") == 0)     a = LV_SCREEN_LOAD_ANIM_FADE_IN;
    else if (strcmp(anim_str, "fade") == 0 || strcmp(anim_str, "fade_on") == 0)
                                                   a = LV_SCREEN_LOAD_ANIM_FADE_ON;
    else if (strcmp(anim_str, "fade_out") == 0)    a = LV_SCREEN_LOAD_ANIM_FADE_OUT;
    else if (strcmp(anim_str, "out_left") == 0)    a = LV_SCREEN_LOAD_ANIM_OUT_LEFT;
    else if (strcmp(anim_str, "out_right") == 0)   a = LV_SCREEN_LOAD_ANIM_OUT_RIGHT;
    else if (strcmp(anim_str, "out_top") == 0)     a = LV_SCREEN_LOAD_ANIM_OUT_TOP;
    else if (strcmp(anim_str, "out_bottom") == 0)  a = LV_SCREEN_LOAD_ANIM_OUT_BOTTOM;
    else {
        lua_lvgl_unlock();
        return luaL_error(L, "unknown load_anim type '%s'", anim_str);
    }
    lv_screen_load_anim(screen, a, ms, delay, auto_del);
    lua_lvgl_unlock();
    lua_pushboolean(L, 1);
    return 1;
}
...
int lua_lvgl_create_widget(lua_State *L, lua_lvgl_obj_type_t type)
{
    ...
    case LUA_LVGL_OBJ_BUTTON:
        ...
        /* Neutralize theme press grow/recolor — expanding dirty areas into
         * live DPI framebuffers reads as button flicker on touch. */
    ...
    if (opts.has_opts) {
        ...
        if (type == LUA_LVGL_OBJ_ARC) {
            ...
            /* Decorative arc (a plain stroke, no drag). LVGL draws the knob
             * unconditionally and the theme owns the indicator colour, but
             * set_style only writes part 0, so neither can be reached from
             * Lua -- both would show up as unstyled artefacts over the
             * background arc. Drop them, and stop the arc swallowing clicks
             * meant for whatever it sits on (lv_arc_constructor adds
             * LV_OBJ_FLAG_CLICKABLE, as lv_spinner then removes). */
            if (!lua_lvgl_get_opt_bool_field(L, 2, "interactive", true)) {
                lv_obj_remove_flag(obj, LV_OBJ_FLAG_CLICKABLE);
                /* lv_arc_constructor disables horizontal scroll chaining.
                 * A decorative ring must not turn a drag begun on it into a
                 * dead gesture: retain its non-interactive surface while
                 * allowing a containing horizontal pager to claim the drag. */
        ...
        /* A floating label is excluded from parent flex/grid positioning.
         * Reapply the explicitly requested frame after its flag is set so the
         * label's position remains absolute within its parent. */
        if (type == LUA_LVGL_OBJ_LABEL &&
            (opts.x != 0 || opts.y != 0) &&
            lua_lvgl_get_opt_bool_field(L, 2, "floating", false)) {
