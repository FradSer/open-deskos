...
static bool lua_lvgl_snapshot_scroll_end_is_incomplete(const lua_lvgl_event_sub_t *sub,
                                                        lv_event_t *event)
{
    const lua_lvgl_obj_record_t *record;

    if (!sub || sub->code != LV_EVENT_SCROLL_END ||
    ...
    /* LVGL sends SCROLL_END(indev) immediately after it starts a release
     * snap. It also sends SCROLL_END(NULL) while a new drag cancels that snap.
     * Neither is a settled pager state, so enqueueing Lua page-dot/quota work
     * here would contend with the bitmap-only transition. */
    return record->scroll_snapshot_restarting ||
...
static void lua_lvgl_event_trampoline(lv_event_t *e)
{
    ...
    if (lua_lvgl_snapshot_scroll_end_is_incomplete(sub, e)) {
        return;
    }
