...
option(OPEN_DESKOS_STRICT_IDF_PATCH "Fail configure when an ESP-IDF patch cannot be verified or applied" ON)

if(NOT DEFINED ENV{IDF_PATH} OR "$ENV{IDF_PATH}" STREQUAL "")
    message(FATAL_ERROR "${OPEN_DESKOS_PROJECT_LOG_PREFIX} IDF_PATH environment variable is not set")
endif()

function(open_deskos_patch_file_replace FILE_PATH OLD_TEXT NEW_TEXT PATCH_NAME)
    if(NOT EXISTS "${FILE_PATH}")
        message(WARNING "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' skipped: file not found: ${FILE_PATH}")
        return()
    endif()
    ...
    if(NOT FIXED_TEXT_OFFSET EQUAL -1)
        message(STATUS "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' already applied")
        return()
    endif()
    ...
    if(OLD_TEXT_OFFSET EQUAL -1)
        if(OPEN_DESKOS_STRICT_IDF_PATCH)
            message(FATAL_ERROR "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' could not be verified or applied: source pattern not found in ${FILE_PATH}")
        endif()
        message(WARNING "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' skipped: source pattern not found in ${FILE_PATH}")
        return()
    endif()

    string(REPLACE "${OLD_TEXT}" "${NEW_TEXT}" FILE_CONTENT "${FILE_CONTENT}")
    file(WRITE "${FILE_PATH}" "${FILE_CONTENT}")
    message(STATUS "${OPEN_DESKOS_PROJECT_LOG_PREFIX} Applied ESP-IDF patch '${PATCH_NAME}'")
endfunction()

function(open_deskos_patch_file_replace_literal FILE_PATH OLD_TEXT NEW_TEXT PATCH_NAME)
    ...
    string(FIND "${FILE_CONTENT}" "${OLD_TEXT}" OLD_TEXT_OFFSET)
    if(OLD_TEXT_OFFSET EQUAL -1)
        if(OPEN_DESKOS_STRICT_IDF_PATCH AND NOT ALLOW_MISSING_SOURCE)
            message(FATAL_ERROR "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' could not be verified or applied: source pattern not found in ${FILE_PATH}")
        endif()
        message(WARNING "${OPEN_DESKOS_PROJECT_LOG_PREFIX} ESP-IDF patch '${PATCH_NAME}' skipped: source pattern not found in ${FILE_PATH}")
        return()
...
open_deskos_patch_file_replace(
...
# Open DeskOS: the usb_iad_desc_size patch targets IDF 5.5's usb_types_ch9.h;
# IDF 6.0 moved/removed that header, so the patch fails to configure. The
# patch is unrelated to P4 display bring-up (USB IAD descriptor size), so
# skip it on IDF 6.0+. Restore if re-pinning to 5.5.
# open_deskos_patch_file_replace(
...
open_deskos_patch_file_replace_literal(
