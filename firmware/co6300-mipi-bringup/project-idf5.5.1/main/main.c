...
#define MIPI_DSI_LCD_H_RES 262  // CO6300 真实 262
...
// === ICNA3312/CO6300 full after-code init (538 cmds) — IDF 5.5.1 native test ===
...
esp_err_t app_lvgl_init() {
    ...
    // 触摸暂禁 (仅验证显示): ZT2628 面板用 CST3530 驱动可能不应答
    // const lvgl_port_touch_cfg_t touch_cfg = { .disp = lvgl_disp, .handle = tp, };
...
void app_main(void) {
    ...
    // ESP_ERROR_CHECK(app_touch_init());  // 暂禁, 仅验证显示
