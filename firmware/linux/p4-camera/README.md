...
Standalone ESP-IDF firmware for the **ESP32-P4 + SC2336 MIPI CSI camera** sub-device. It keeps face detection, owner recognition, feature storage, and USB metadata transport inside `firmware/linux/` so the Orange Pi CM5 does not need a companion machine.
...
## Owner-only Screen Access

The P4 uses the ESP-DL MSR/MNP face detector and MFN face-feature model. The screen remains locked unless the **current P4-selected face** matches an explicitly enrolled local owner feature at or above the P4 threshold.

- No face, unknown face, malformed metadata, stale metadata, or a failed P4 link leaves the CM5 screen covered by the privacy shield.
...
A successful future recognition reports `unlocked: true`; the privacy shield then disappears. Zero faces, multiple faces, an invalid face, a non-match, or an expired confirmation window never changes the owner database and never unlocks the shell.
