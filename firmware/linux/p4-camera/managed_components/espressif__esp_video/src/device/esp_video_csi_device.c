...
static esp_err_t init_config(struct esp_video *video)
{
    ...
    if (sensor_format.isp_info) {
        ...
        if (!ESP_CHIP_REV_ABOVE(efuse_hal_chip_revision(), 300)) {
            /* P4 v1.x cannot convert formats in the CSI bridge. Keep the
             * physical capture stream RAW8 and convert a copied frame in the
             * inference worker before ESP-DL consumes it. */
...
#if CONFIG_IDF_TARGET_ESP32P4
    if (!ESP_CHIP_REV_ABOVE(efuse_hal_chip_revision(), 300)) {
        csi_config.output_data_color_type = csi_video->state.in_color;
    }
#endif
