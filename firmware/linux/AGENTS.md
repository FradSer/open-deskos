...
- `tests/` contains mixed-language BDD features, Node contracts, smoke/layout
  checks, and Electron E2E. `remote-bridge/`, `p4-camera/`, `remote-control/`,
  and `face-agent/` are independently executable subsystems with scoped guides.
...
node --test remote-bridge/test/*.test.js  # Remote Bridge protocol/service tests
```
