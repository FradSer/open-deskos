...
`face_service.py` is the Python service overlay installed by `../scripts/cm5-install.sh`. `systemd/open-deskos-face-agent.service` runs it as the graphical user’s systemd service. Electron reads only `http://127.0.0.1:8790/status` through `../src/face-agent-status.js`; the behavior contract and executable checks live in `../tests/features/linux-shell.feature` and `../tests/face-agent-*.test.js`.
...
From `firmware/linux/`, verify service changes with:

```sh
pnpm test
python3 -m py_compile face-agent/face_service.py
```
