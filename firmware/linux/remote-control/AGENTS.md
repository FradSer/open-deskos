...
Run `pnpm test` from `firmware/linux/` for repository-level contracts. The Linux `remote-bridge` tests validate the host-side protocol; they do not substitute for flashing and testing the USB-OTG device.
