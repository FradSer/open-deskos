...
| P-2 | 应用平台（概念归档，后续重构） | 用户用自己的 prompt 生成 app，或安装扩展包（已归档至 docs/archive/app-center/） | §6 |
...
3. **§四触摸 IC 型号**:SPEC 写 CST3530,但在手实物 bring-up 实测为 **ZT2628**(I2C 0x18/0x58,工作正常,见 [co6300-mipi-bringup/README](../../firmware/co6300-mipi-bringup/README.md));两者同为 I2C 电容触控,驱动工作量相当。**以实物实测为准,实施 021 任务时先探测确认**。
...
所有 UI 可见性最终堵在同一个地方。四个硬门(HG = hard gate),每个带当前状态与 pivot:

| 门 | 内容 | 当前状态 | 判据 | pivot(失败分支) |
|---|---|---|---|---|
| **HG-1** | CO6300 面板点亮(= 06-13 计划 task-025 / LUMINA G5) | **NEVER LIT**。DSI command link 挂起,根因收窄到 P4 侧(cmd_ack 无超时自旋);`cmd_ack=false` 实验补丁已打、待烧录;硬件疑点:DSI_REXT 4.02kΩ、VDD_MIPI_DPHY 2.5V。详见 [bring-up 档案](../../firmware/co6300-mipi-bringup/README.md) | 538-cmd init 完成且出图,单缓冲 flush ≥30fps | ① 换 P4 模块(唯一未换过的公共件);② 换面板供应商:Tailor Pixels 同尺寸 MIPI 版(RM690C0,Raydium 家族有成熟开源驱动),代价是自行解决与转接板的对接([SPEC §八](SPEC-MVP.md) 已论证);QSPI 路线对本 FFC 不可行 |
...
- M1"打字机"里程碑(§11.2)**显式无屏可跑**:语音键→说话→Mac 上屏,状态用 LED/日志表达。旗舰功能与 HG-1 解耦。
- OS 外壳/widget/沙盒全部先跑 **LVGL host simulator**(SDL,928×262 与 262×928 双目标,任务 NT-2),HG-1 通过后一次性上真机。
- HG-1 本身当前只差"烧录验证"这一步,是全仓库优先级最高的单项行动。
...
> **归档说明（2026-08）**：App Center 独立主屏页面与旧目录存根已被移出当前活跃概念与主屏排版，代码与文档完整归档在 `docs/archive/app-center/`，后续将基于 v2.0 模块化插件体系（`core/plugin_registry.lua`）进行统一重构。
...
```
泳道 A 硬件门:   HG-1(烧录验证→硬件排查→pivot)… HG-2 … HG-3 … HG-4
...
| 里程碑 | 内容 | 验收判据 |
|---|---|---|
| **M0 硬件解锁** | HG-1~4 四个 spike/gate,与一切并行,**HG-1 立即执行** | 各门判据见 §4 表;每门出书面结论(通过/`pivot` 分支选择) |
...
| **M2 桌面** | esp-claw fork 基座 + 模拟器 + 外壳(状态栏/widget 主屏/编辑模式)+ 沙盒硬化 + 时钟/连接状态两个内置 widget + 设置 | 模拟器与真机(HG-1 后)双跑;横置 ≥30fps(HG-4 判据);触摸翻页/进出 app/编辑模式全通;上电→可用 <3s |
...
| **修订** | 004(UI 状态机 → 外壳导航 FSM:主屏/app/overlay/编辑四态,仍宿主机可测)、020(显示实现改走 fork 的 `setup_device.c` lcd factory-entry,重设计 FR-2)、021(触摸 IC 先实测定型 ZT2628/CST3530)、022(组合根改为 fork 的 `app_main` 集成 + SPEC §十二任务表) | Red-Green 配对与依赖关系保持 |
| **取代** | 013(P4 from-scratch 骨架)→ **NT-1 esp-claw fork bootstrap** | §2 裁决的落点 |
| **等价** | 025 = HG-1;026 = HG-2 | 已在执行序列最前 |
...
2. **触摸 IC 实测定型**:ZT2628 vs CST3530(§2 修订 3)。
...
5. **HG-1 pivot 触发线**:`cmd_ack=false` 实验 + 两项硬件检查 + 换模块后仍不亮,是否立即切 Tailor Pixels RM690C0 面板(开源驱动家族)——建议给 HG-1 设 2 周硬时限。
6. **设备命名**:本文用 "Open DeskOS" 指代固件/系统;产品名沿用 Open DeskOS。
