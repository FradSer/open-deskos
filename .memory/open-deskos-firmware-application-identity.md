...
Open DeskOS 的生产固件工程路径是 `research/esp32-p4-c6-deskos/firmware/application/open_deskos`，ESP-IDF project identity 是 `open_deskos`。上游 `edge_agent` 名称不再出现在当前代码、构建和文档路径中。独立的 `application/mcp_server_point` 上游样例也已移除；固件应用目录只保留 `open_deskos` 和 `c6_espnow_bridge`。
...
**How to apply:**
- 固件构建从 `research/esp32-p4-c6-deskos/firmware/application/open_deskos` 执行。
