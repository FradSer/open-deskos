...
The Pi Sessions Session Detail reproduces Pi's own reading, including the colours Pi itself gives a reply: Markdown headings, links, inline code and quotes, syntax-highlighted code fences, and diff additions and removals. That content is quoted, not composed by the desk, so it keeps Pi's roles rather than being recoloured to hide them; an operator reading a session on the desk must see what the same session looks like in Pi.

The palette is declared once as `--pi-*` variables scoped to `.pi-app-wrapper` in `runtime/linux/src/renderer/shell.css`, using the values of Pi's `dark` theme, and is bounded to Session Event bodies. It is not a second design system: the page's own surfaces, controls, and states stay on the `--odk-*` tokens above, and no `--pi-*` value may be used outside the transcript. See `runtime/linux/docs/adr/0015-pi-sessions-live-only-and-pi-reading.md`.
