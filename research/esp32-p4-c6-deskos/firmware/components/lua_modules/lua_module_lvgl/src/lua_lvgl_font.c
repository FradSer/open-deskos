...
#endif
    /* Voice UI / claw-fail paths call lvgl.init before app_claw sets the
     * fatfs data root. Built-in fonts still work; skip the S: driver. */
    if (!s_lvgl.data_root[0]) {
        ESP_LOGW(TAG, "data root unset — skipping LVGL fs driver (builtin fonts only)");
        return ESP_OK;
    }
...
/* Text-metrics harness: measure a string with this font WITHOUT creating a
 * label — the pixel size drives fixed-height layout math so text never
 * overlaps its siblings. Returns width, height (both integers). An optional
 * max_width (2nd arg) wraps the text at that width and returns the wrapped
 * height, matching what LVGL renders for a multi-line label. */
static int lua_lvgl_font_measure(lua_State *L)
{
    lua_lvgl_font_ud_t *ud = lua_lvgl_check_font(L, 1);
    const char *text = luaL_checkstring(L, 2);
    int32_t max_width = (int32_t)luaL_optinteger(L, 3, 0);
    esp_err_t err = lua_lvgl_lock();
    lv_font_t *font;
    const char *font_error = NULL;
    ...
    lv_text_get_size(&size, text, font, 0, 0,
                     max_width > 0 ? max_width : LV_COORD_MAX, LV_TEXT_FLAG_NONE);
    lua_lvgl_unlock();
...
/* Return the actual bitmap bounds, offsets, and shared baseline of a
 * one-codepoint glyph. Text advance alone is insufficient for visual
 * centering: icon fonts commonly leave asymmetric side bearings around their
 * drawn outline. */
static bool lua_lvgl_decode_one_utf8(const char *text, size_t text_len, uint32_t *out_codepoint)
{
    ...
    if (bytes[0] < 0x80) {
        ...
        expected_len = 2;
        if (text_len != expected_len || (bytes[1] & 0xC0) != 0x80) {
            return false;
        }
    ...
    if ((expected_len == 2 && codepoint < 0x80) ||
        (expected_len == 3 && codepoint < 0x800) ||
        (expected_len == 4 && (codepoint < 0x10000 || codepoint > 0x10FFFF)) ||
...
static int lua_lvgl_font_glyph_bounds(lua_State *L)
{
    lua_lvgl_font_ud_t *ud = lua_lvgl_check_font(L, 1);
    size_t text_len;
    ...
    if (text_len == 0) {
        return luaL_argerror(L, 2, "glyph must not be empty");
    }
    if (err != ESP_OK) {
        return lua_lvgl_error_esp(L, "lock", err);
    }
    ...
    if (!lua_lvgl_decode_one_utf8(text, text_len, &codepoint) ||
        !lv_font_get_glyph_dsc(font, &glyph, codepoint, 0)) {
        lua_lvgl_unlock();
        return luaL_error(L, "glyph must contain one supported codepoint");
    }
    lua_pushinteger(L, glyph.adv_w);
    lua_pushinteger(L, glyph.box_w);
    lua_pushinteger(L, glyph.box_h);
    lua_pushinteger(L, glyph.ofs_x);
    lua_pushinteger(L, glyph.ofs_y);
    lua_pushinteger(L, font->base_line);
    lua_lvgl_unlock();
    return 6;
}

static int lua_lvgl_font_line_height(lua_State *L)
{
    lua_lvgl_font_ud_t *ud = lua_lvgl_check_font(L, 1);
    esp_err_t err = lua_lvgl_lock();
    lv_font_t *font;
...
void lua_lvgl_register_font_metatable(lua_State *L)
{
    static const luaL_Reg font_methods[] = {
        ...
        {"measure", lua_lvgl_font_measure},
        {"glyph_bounds", lua_lvgl_font_glyph_bounds},
        {"line_height", lua_lvgl_font_line_height},
        {NULL, NULL},
    };
