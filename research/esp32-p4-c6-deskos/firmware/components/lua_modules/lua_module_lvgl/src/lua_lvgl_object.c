...
void lua_lvgl_record_release_resources(lua_lvgl_obj_record_t *record)
{
    ...
    if (record->data_size == LUA_LVGL_SNAPSHOT_DATA_MAGIC && record->data != NULL) {
        /* Snapshot image widget: data points at lua_lvgl_snapshot_t whose
         * draw_buf is heap-allocated by lv_snapshot_take — must be released
         * with lv_draw_buf_destroy, not free(). */
        lua_lvgl_snapshot_t *snap = (lua_lvgl_snapshot_t *)record->data;
        if (snap->draw_buf != NULL) {
            lv_draw_buf_destroy(snap->draw_buf);
            snap->draw_buf = NULL;
        }
        free(record->data);
        record->data = NULL;
