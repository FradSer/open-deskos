...
esp_err_t wifi_manager_init(void)
{
    ...
    /* Open DeskOS: esp_wifi_init() drives the esp-hosted transport, which fails
     * (ESP_FAIL) when the C6 co-processor / esp-hosted slave is not up (HG-2).
     * The upstream ESP_ERROR_CHECK here aborts -> SW_CPU_RESET boot-loop,
     * preventing the headless app platform (serial console, packages
     * partition, cerb services) from starting. Degrade instead: return the
     * error so the caller (app_main) can log and continue. The downstream
     * event-handler/timer registration is skipped on this failure path. */
    esp_err_t wifi_init_err = esp_wifi_init(&cfg);
    if (wifi_init_err != ESP_OK) {
        ESP_LOGW(TAG, "esp_wifi_init failed (esp-hosted/C6 not ready?): %s",
