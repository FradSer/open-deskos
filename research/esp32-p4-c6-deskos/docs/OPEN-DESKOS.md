...
> Status: historical parallel research. This document describes the prior P4+C6 DeskOS device OS and is retained for reproducibility. It does not define the active CM5/Linux Open DeskOS product, boot path, UI, peripheral acceptance, or release gates. See [../../../PRODUCT.md](../../../PRODUCT.md) for the active product.
>
> 2026-07-07 直接撰写（非 brainstorming 流程产物）。本文把当时的 Open DeskOS 从“以 HID 模拟为核心的富功能输入设备”重新定位为桌面 OS 设备，统一收编既有的 MVP 规格、App Center 重设计与 ESP-NOW 子系统，并给出当时的硬门、里程碑和任务映射。

## 1. 产品定义

**Open DeskOS**:运行在 ESP32-P4 + ESP32-C6 上的桌面操作系统,设备常驻键盘旁(928×262 横置触摸条形态),经 USB-C 连接 Mac,搭配 macOS 常驻 app 构成完整体验。

> CM5/S31 的后续实验位于 [../../../experiments/cm5-s31-gateway/README.md](../../../experiments/cm5-s31-gateway/README.md)。它不改变本历史 P4+C6 研究记录的结论。
