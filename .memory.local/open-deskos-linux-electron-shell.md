...
- **CM5 内核维护审计(2026-08-29)**: 当前运行 `6.1.43-rockchip-rk3588`，由 `linux-image-current-rockchip-rk3588=1.0.0` 提供；Orange Pi vendor kernel、DTB(`rk3588s-orangepi-cm5-tablet.dtb`)、Mali/RK NPU/HDMI/BCM Wi-Fi 驱动都依赖该板级树。APT 没有更新的 vendor kernel 候选；Debian generic `linux-image-arm64=6.1.176`/backports 6.12 虽可安装，但不能作为无验证替换。除非 Orange Pi 发布匹配 CM5 Tablet 的新 kernel/BSP，或有明确 kernel bug/security 要求，不升级到 generic kernel；先保持当前内核并备份 `/boot`、`orangepiEnv.txt` 与 DTB。
