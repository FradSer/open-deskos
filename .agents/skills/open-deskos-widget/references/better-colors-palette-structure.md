...
What a color system is made of, before any values exist. For computing the values see [palette-generation.md](better-colors-palette-generation.md); for what to call them see [token-naming.md](better-colors-token-naming.md).
...
Convention constrains status hues before taste does. Red reads as danger, amber as warning, green as success. See the cultural exceptions in [color-usage.md](better-colors-color-usage.md).
