...
Producing the values once the structure is decided. For which ramps to build and what each step is for, see [palette-structure.md](better-colors-palette-structure.md).
...
- **Contrast does not survive the mirror.** A pair passing in light mode can fail reversed, because contrast is not symmetric. Recheck every foreground against its real background in both appearances ([contrast.md](better-colors-contrast.md)).
