...
**For a genuinely new color system, `oklch()` is the best default.** Even lightness steps stay even, and a fixed hue stays fixed. See [palette-generation.md](better-colors-palette-generation.md).
...
- **`light-dark()`** puts both appearances in one declaration. See [palette-generation.md](better-colors-palette-generation.md).
