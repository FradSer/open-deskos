...
Naming is what makes a palette usable by anyone who did not build it. For which ramps exist and what each step does, see [palette-structure.md](better-colors-palette-structure.md).
...
Every one of them is a case of **Use a token only in its role**. See [color-usage.md](better-colors-color-usage.md).
