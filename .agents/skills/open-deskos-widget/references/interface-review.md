...
The whole invocation is the target, so `/interface-review pr 482` reviews pull request 482. [Scope resolution](interface-review-scope-resolution.md) holds the accepted targets and how each resolves.
...
State the repository facts you found, then offer the routes and wait. [Nothing to review](interface-review-scope-resolution.md#nothing-to-review) holds the facts to gather:
...
Review at most five consumers, ordered by [the rule in Scope resolution](interface-review-scope-resolution.md#expanding-to-consumers), then state how many you did not expand. A sweep with no bound cannot support the coverage it claims, and an unstated cutoff reads as completeness.

### 4. Read the removed lines

Regressions are invisible in the post-change state. Read the `-` side of every hunk against [Removed signals](interface-review-removed-signals.md).
