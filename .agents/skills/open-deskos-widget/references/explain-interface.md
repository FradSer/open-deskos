...
| --- | --- | --- |
| How was this **site** built? | The frontend: framework and rendering strategy, styling system, component library, tokens, the type, spacing and color systems, motion, breakpoints, how fonts and images are served | [read-the-system.md](explain-interface-read-the-system.md) |
| How was **this** built? | The layer stack behind one effect, in paint order, with the technique on each layer | [find-the-effect.md](explain-interface-find-the-effect.md) |
...
Without a browser, [no-browser.md](explain-interface-no-browser.md) holds the fetch method.
...
So where the page is live, ask for the URL. One command replaces the whole estimate. [from-an-image.md](explain-interface-from-an-image.md) holds the method for when it is not.
...
[find-the-effect.md](explain-interface-find-the-effect.md) holds the search recipes. It also names the three things that otherwise cost you the answer: pseudo-element layers, the idle values animation libraries leave behind and generated stop lists.
