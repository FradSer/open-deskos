...
Also run `pnpm test` from `runtime/linux/` after changes that affect Electron integration. The bridge requires an absolute `XDG_RUNTIME_DIR`; the service installer substitutes `__OPEN_DESKOS_REMOTE_BRIDGE_DIR__` with the installed integration path.
