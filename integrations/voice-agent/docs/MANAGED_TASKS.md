...
Create a private file on CM5 and set `ODESK_TASK_TARGETS_FILE` in the voice service's environment — `voice-agent.env`, or a `open-deskos-voice-agent.service.d/*.conf` drop-in — to its absolute path:
...
- Every other command carries the session's **own** project, exactly as the list reported it. A broader scope resolves a session only in the list; an identity call that reuses the root is refused as an unknown session, which keeps one project from reaching a session under a different one. The daemon reports a project as its real path, and it is the only side that resolves the configured roots on disk: the coordinator carries the project it was given verbatim, so a target whose configured root is itself a symlink accepts the project the list reported without any second path mapping on the desk.
...
- An unconfigured or empty target list is reported as needing device-local configuration (`ODESK_TASK_TARGETS_FILE` in the voice service environment, which is `voice-agent.env` or a unit drop-in — a drop-in wins over the env file, so declare it in exactly one — and the host's `pi-tasks.json`). The coordinator never invents a host, project, or root to have something to control.
