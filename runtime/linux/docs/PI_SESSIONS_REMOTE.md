...
- The local collector's existing discovery limitations still apply. It reads processes visible to the Mac login account, and metadata may be absent. It does not read another user's Pi history. The remote CLI fails rather than reporting idle when process-table inspection fails; ensure the SSH account PATH includes the system `ps` and `lsof` utilities.
