...
Accepted. Supersedes ADR-0007's control row and ADR-0008's All-filter default and Overview-as-chooser framing. ADR-0012's bounded Markdown result body is extended, not replaced. ADR-0021 extends this record: the transcript's own surfaces — a prompt's band and a tool call's box carrying the outcome Pi recorded — join the Markdown, syntax, and diff colours borrowed here, so the `--pi-*` exception now covers the containers as well as the text inside them.
