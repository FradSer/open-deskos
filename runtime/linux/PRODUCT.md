...
The active implementation focus for Open DeskOS. It is not yet a committed supported product line: P4/C6 firmware, P4 camera, C6 gateway, and installable packages remain preserved experiments with separate acceptance decisions. The binding fidelity duty is a calm, truthful desk companion that remains useful when every optional integration is absent.
...
- Five base pages with threshold-based horizontal swipe, visible page context, and State Bar dot sync: Today / Home / Reading / Pi Sessions / Usage. Installed interactive Apps add individual pages; user Widgets share the display grids.
...
- The State Bar uses a wifi-only network reachability indicator that draws a struck wifi glyph while the network is unreachable; OpenCode Go and Remote Link states remain available in their dedicated surfaces without duplicating status text in the bar.
- Today renders one statement per `ctx.briefing` contribution and owns only their typography: plain connectives stay quiet while emphasized signals carry their own inline icon. Any plugin may contribute, so the first page is composed by the plugin system rather than by the page itself, and a Plugin that contributes nothing leaves `No desk briefing is available.`
...
- Widget taps use `open-app` intent only for declared built-in views; `display-only` Widgets remain truthful and do not pretend to be launchers. The application lifecycle service installs user Widgets into requested desktop grid positions and exposes interactive Apps as individual pages, without a separate user-application collection page. Back and Escape always return to the exact source page and context.
...
- Which optional experiment—P4 camera or C6 gateway—earns a supported provider contract after its own acceptance gate. The local user-application lifecycle is implemented and supported within its package limits; it does not imply marketplace, native, background, or network capabilities.
...
1. **Glance first, dive second.** The State Bar, Today, and Widgets state what is true; focused built-in views add depth only when needed. Today speaks only in statements a plugin has substantiated.
...
4. **Core before experiment.** The shell remains usable without P4 camera, C6 gateway, or Remote Bridge; local user packages are optional and independently managed.
