...
## 6. Historical HTTP and SSE proposal
...
| `/events` | `GET` | Historical SSE proposal; not used by the active CM5 runtime | `text/event-stream` |
...
## 7. Active CM5 integration boundary

The active runtime does not subscribe to `/events`, consume raw FaceID records, or unlock protected UI. It calls only the fixed loopback `http://127.0.0.1:8790/status` endpoint through `runtime/linux/src/face-agent-status.js`; that normalizer exposes a bounded, fail-closed status snapshot to Electron.

A future experiment may add a visual status consumer only after an explicit product decision. It must not expose biometric records, create identity remotely, or gate base-shell installation, startup, touch, keyboard, or normal view visibility.
